...
- For the Rock → Flying → Fighting triangle, the "Flying" corner must carry the Flying type **from the unevolved starter the player actually receives** (needed for the effectiveness math against Rock/Fighting to hold from level 5 on), but its secondary type is free to vary with whichever line is drawn — Normal or Grass in the pinned Gen 1–5 pool.
...
- **Starter eligibility:** a corner's pool is every 3-stage evolution line whose **base (unevolved) stage** carries the corner's type, final BST ≤ 540, within the Roster Cap — not just canonical starters (Krookodile, Alakazam, or Crobat can be a starter; that's the point). A line whose type only appears after evolving (e.g. a pure-Fire base that gains Flying at its final stage) does not qualify — the starter must read as its corner's type the moment the player receives it. One hard filter: the line must be **completable by leveling alone** — lines requiring stones, Link Cables, or friendship to finish are excluded from starter pools (they remain in the regular dex).
