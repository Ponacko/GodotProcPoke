...
/// <summary>
/// Per-area encounter tables (GDD §6.3), Gen 5 slot model. The pass assigns methods and base levels from 6a,
/// fills slots from the dex by affinity and BST rarity tier, and adds Special Encounter Overlays. All draws use
/// the <c>"encounters"</c> stream.
/// </summary>
        ...
        foreach (var areaId in availabilityOrder)
        {
                ...
                && fraction >= 0.5)
            {
                overlay = FillTable(EncounterMethod.Land, SlotLayouts.Land, landTiers, baseLevel, 5, 0, rng,
                    landTiers.RareAndVeryRare) with { HiddenAbilityChance = 0.5 };
            }
